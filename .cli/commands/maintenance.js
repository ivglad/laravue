...
export const registerMaintenanceCommands = (program) => {
  ...
  program
    .command("distclean")
    .description("Удаление всех созданных файлов")
    .action(async () => {
      ...
      try {
        runner.addOutputLine(
          COLOR.INFO_SYMBOL("Начало процесса удаления всех созданных файлов...")
        );

        // Сначала вызовем команду clean
        const cleanCommand = program.commands.find(
          (cmd) => cmd.name() === "clean"
        );

        if (cleanCommand) {
          runner.setText(COLOR.PROCESS("Выполнение команды clean..."));
          runner.addOutputLine(
            COLOR.INFO_SYMBOL("Запуск очистки временных файлов...")
          );
          await cleanCommand.action();
          runner.addOutputLine(
            COLOR.DONE("Очистка временных файлов завершена")
          );
        }
        ...
        await findAndDeleteFiles("cache");

        // Удаление node_modules и vendor
        const findAndDeleteDirs = async (dirName) => {
          try {
            runner.setText(
              COLOR.PROCESS(`Поиск каталогов ${COLOR.DIR(dirName)}...`)
            );
            const findCmd = `find ${PROJECT_DIR} -name "${dirName}" -type d`;
            const dirsStr = execSync(findCmd, {
              encoding: "utf8",
              stdio: "pipe",
            });

            if (dirsStr.trim()) {
              const dirs = dirsStr.trim().split(os.EOL);
              runner.setText(
                COLOR.PROCESS(
                  `Удаление ${COLOR.VALUE(dirs.length)} каталогов ${COLOR.DIR(
                    dirName
                  )}...`
                )
              );
              ...
              for (const dir of dirs) {
                if (dir.trim()) {
                  try {
                    ...
                    runner.addOutputLine(
                      COLOR.DONE(
                        `Удален каталог: ${path.relative(PROJECT_DIR, dir)}`
                      )
                    );
                  } catch (error) {
                    runner.addOutputLine(
                      COLOR.WARNING_SYMBOL(
                        `Не удалось удалить ${dir}: ${error.message}`
                      )
                    );
                  }
                }
              ...
              runner.addOutputLine(
                COLOR.SKIPPED(`Каталоги ${dirName} не найдены`)
              );
            }
          } catch (error) {
            runner.addOutputLine(
              COLOR.WARNING_SYMBOL(
                `Ошибка при поиске каталогов ${dirName}: ${error.message}`
              )
            );
          }
        };

        await findAndDeleteDirs("node_modules");
        await findAndDeleteDirs("vendor");

        runner.succeed("Все созданные файлы успешно удалены");
      } catch (error) {
        runner.fail(`Ошибка при удалении файлов: ${error.message}`);
        console.error(
          COLOR.ERROR_SYMBOL(`Ошибка при удалении файлов: ${error.message}`)
        );
        process.exit(1);
      }
    });
